# Use RepeatedKFold to do 5 x 10 fold validation of PD model-2 regularized logistic regression model regression model

# Inputs: Weighted_eQTL_matrix.txt
# The full tissue specific eQTL table

# Outputs: 
# 10fold_PDModel-2_hyperparameteres_model{count}weights.txt where count = 1 to 50 contains the model components and their weights for each randomized model
# AUC_results_of_50_regularized_predictors_with_PDmodel-2_hyperparameteres.txt contains the AUC results for the 50 randomized predictors
# Preformance_of_50_regularized_predictors_with_PDmodel-2_hyperparameteres.txt contains the performance results for the 50 randomized predictors
...
#        best_clf = clf
	...
	m_name = 'data/10fold_PDModel-2_hyperparameteres_model' + str(count) + 'weights.txt'
...
AUC_out.to_csv("data/AUC_results_of_50_regularized_predictors_with_PDmodel-2_hyperparameteres.txt", sep='\t',index=False, line_terminator='\n')
...
f= open("data/Preformance_of_50_regularized_predictors_with_PDmodel-2_hyperparameteres.txt","w+")
