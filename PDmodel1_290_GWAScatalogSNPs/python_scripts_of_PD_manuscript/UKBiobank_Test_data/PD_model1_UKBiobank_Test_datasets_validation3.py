...
#
# This python script is to validate the PD model-1 predictor model with the 30 sets of Test data derived from UK Biobank Parkinson's disease data
#
# Input datasets:
# After_0.05_MMann_selected_header.txt (The Mann Whitney (FDR = 0.5) selected header columns of the PD tissue-specific eQTL matrix derived from WTCCC)
...
# PD_GWAS_Nalls_finalfull_Mann0.05_lg_saga_elect0.6C0.5max800_beta_20122020features.txt (Feature columes of PD model-1)
# PD_GWAS_Nalls_finalfull_Mann0.05_lg_saga_elect0.6C0.5max800_beta_20122020.sav (Saved PD model-1)
# PD_UKBio_full_NallsGWAS_a_41SNP_controls_beta_eQTL_table1.txt to PD_UKBio_full_NallsGWAS_a_41SNP_controls_beta_eQTL_table30.txt (PD control Test datasets derived from UK Biobank Parkinson's disease data)
# 
# Output data:
...
org_table = pd.read_csv("data/After_0.05_MMann_selected_header.txt", sep="\t")
...
eQTL_table = pd.read_csv("data/PD_UKBio_full_NallsGWAS_a_41SNP_cases_beta_928_eQTL_table.txt", sep="\t")
...
for p in num:
	control_file = "data/PD_UKBio_full_NallsGWAS_a_41SNP_controls_beta_eQTL_table" + str(p) + ".txt"
