# This program to combine the 50 model results from C_10fold_PDmodel-1_hyperparameters_50_predictors 
# and create a tissue model weights matrix of the 50 predictor built with PD_model-1 hyperparameters

# Input files : 10fold_PDModel-1_hyperparameteres_model{count}weights.txt where count = 1 to 50 contains the model components and their weights for each randomized model

# Output file : tissue_results_50models.txt a tissue model weights matrix of the 50 predictor created with PD_model-1 hyperparameters
...
for Num in range(50):
	Num1 = Num + 1
	filename = 'data/10fold_PDModel-1_hyperparameteres_model' + str(Num1) + 'weights.txt'
	organ_value = load_table(filename)
	interlines.append(organ_value)

organ_data = pd.DataFrame(interlines)

organ_data.to_csv('data/tissue_results_50models.txt',  index = None, sep='\t', header=True) 
